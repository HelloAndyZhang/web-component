...
export default class Component extends HTMLElement {
  constructor(template, props = {}, state = {}) {
    ...
    tmp = ce('template')
    tmp.innerHTML =
      '<link href="common.css" rel="stylesheet"/>' + svg.cloneNode(true).outerHTML + template
  ...
  // parent component rendered before current component
  ...
  _xEventBinding() {
    ...
    map.forEach((event) => {
      forEach(this.qsa(`[${event}]`), (dom) => {
        ...
        const params =
          property.indexOf('(') > 0 ? property.replace(/.*\((.*)\)/, '$1').split(',') : []
        const args = []
